...
class Gradient:
    ...
    def epoch(self, X, y, w, M):
        ...
        num_epoch_step = int(np.ceil(cnt / M))
        for step in range(num_epoch_step):
            lo_bound, hi_bound = step * M, min((step + 1) * M, cnt)
            Xtrain, ytrain = X[lo_bound:hi_bound], y[lo_bound:hi_bound]

            gradient = self.gradient(Xtrain, ytrain, w)

            w = w - self.step_size() * gradient

            self.update_step(gradient)
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print(
            "usage: python %s filename mode eta tau batch_size epochcnt" % sys.argv[0]
        )
    else:
        main(sys.argv)
