...
def file_reader(path: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_house = pd.read_json(house_file_path, orient="records")

    return df_price, df_house
    ### END CODE HERE
...
def dataframe_merger(prices: pd.DataFrame, house_data: pd.DataFrame) -> pd.DataFrame:
    ...
    df_merged = prices.merge(right=house_data, how="left", on=["date", "postcode", "bedrooms", "area", "yr_built"])
...
def correct_distance_unit(df: pd.DataFrame) -> None:
    ...
    ### START CODE HERE
    idx_false = df[df["distance"] >= 100].index
    df.loc[idx_false, "distance"] = df.loc[idx_false, "distance"] / 10e3
    ### END CODE HERE
...
def typo_fixer(df: pd.DataFrame, threshold: float, correct_condition_values: List[str]) -> None:
    ...
    ### START CODE HERE
    def _f_get_closest(r):
        out = process.extractOne(r.condition, correct_condition_values, scorer=fuzz.ratio)

        return pd.Series({"index": r.name, "closest": out[0], "similarity_scores": out[1]})

    out = df.apply(_f_get_closest, axis=1).set_index("index")

    df.loc[:, "similarity_scores"] = out["similarity_scores"]

    out = out[out["similarity_scores"] > threshold]

    df.loc[out.index, "condition"] = out["closest"]
    ### END CODE HERE
...
def batch_creator(df: pd.DataFrame, context: FileDataContext, data_source_name: str) -> BatchRequest:
    ...
    try:
        ...
        test_data = datasource.add_dataframe_asset(name=test_asset_name)
    except ValueError:
        # The Data Asset already exists
        test_data = datasource.get_asset(test_asset_name)

    batch_request = test_data.build_batch_request(dataframe=df)
...
def create_checkpoint(
    context: FileDataContext, batch_request: BatchRequest, checkpoint_name: str, expectation_suite_name: str, run_name: str
) -> CheckpointResult:
    """
...
def separate_X_and_y(df: pd.DataFrame, target: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    df = (
        df.copy()
    )  # By copying the DataFrame we protect the original DataFrame in case we make errors in the following assignments. This way, we can always restart from this step.
...
def impute_missing(df: pd.DataFrame) -> None:
    ...
    ### START CODE HERE
    out = imp.fit_transform(df.loc[:, included_columns])
    df.loc[:, included_columns] = out[:, :-2]

    df["missingindicator_sqft_living15"] = out[:, -2]
    df["missingindicator_sqft_lot15"] = out[:, -1]
    ### END CODE HERE
...
def condition_encoder(df: pd.DataFrame) -> None:
    ...
    df.loc[:, col_name] = df[col_name].map({"poor": 1, "tolerable": 2, "satisfactory": 3, "good": 4, "excellent": 5})
    ### END CODE HERE
...
def target_encode(df: pd.DataFrame, columns: List[str], target: Optional[pd.Series] = None, encoder: Optional[TargetEncoder] = None) -> TargetEncoder:
    ...
    if target is not None:
        encoder.fit(df[columns].to_numpy(), target.to_numpy().reshape(-1, 1))
    df.loc[:, columns] = encoder.transform(df[columns].to_numpy())
    ### END CODE HERE
...
def store_encoder(encoder: TargetEncoder, encoder_file_path: Path) -> None:
    ...
    ### START CODE HERE
    with open(encoder_file_path, "wb+") as fp:
...
def etl(
    ...
    chkpt_result = create_checkpoint(
        context=context,
    ...
    if chkpt_result.success is True:
        print("Some GC validation(s) failed.")
