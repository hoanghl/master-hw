/* $Id: forces.c 18 2008-09-16 09:54:33Z aakurone $ */
  ...
  *virial = 0.0;
  ...
  for (i = 1; i <= N; i++)
  {
    ...
    for (jj = 1; jj <= nneighboursi; jj++)
    {
      ...
      /* Get distance */
      dx = x[j].x - x[i].x;
      if (periodic.x == 1 && dx > half.x)
        dx = dx - size.x;
      if (periodic.x == 1 && dx < -half.x)
        dx = dx + size.x;

      dy = x[j].y - x[i].y;
      if (periodic.y == 1 && dy > half.y)
        dy = dy - size.y;
      if (periodic.y == 1 && dy < -half.y)
        dy = dy + size.y;

      dz = x[j].z - x[i].z;
      if (periodic.z == 1 && dz > half.z)
        dz = dz - size.z;
      if (periodic.z == 1 && dz < -half.z)
        dz = dz + size.z;
      ...
      if (rsq <= rpotcutsq)
      {
        ...
        V = 0.5 * 0.5 * morseD * (help2 - 2.0 * help1);
        Epot[i] = Epot[i] + V;
        Epot[j] = Epot[j] + V;

        /* Calculate forces. Take care with signs /* */

        /* First get dV/dr in units of eV/� */
        dVdr = -0.5 * morseD * (help2 * (-2.0 * morsealpha) - 2.0 * help1 * (-morsealpha));
        ...
        *virial = *virial + dVdr * (dx / r * dx + dy / r * dy + dz / r * dz);
        ...
        dVdr = dVdr * e / lunit;
        /* Then get a=F/m in units of �/fs^2 */
